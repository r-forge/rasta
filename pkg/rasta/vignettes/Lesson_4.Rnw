...
The package sp provides classes for spatial-only geometries, such as \texttt{SpatialPoints} (for points), and combinations of geometries and attribute data, such as a \texttt{SpatialPointsDataFrame}. The following data classes are available for spatial vector data \citep{Edzer:2005ux}:
