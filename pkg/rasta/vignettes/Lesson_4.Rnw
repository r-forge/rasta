...
@
Feel free to export the updated lines to Google Earth or to inspect the contents of the \texttt{data} slot of the object \texttt{mylinesdf} (i.e. type \texttt{mylines@data}).
...
Plot results using a few more advanced options of \texttt{spplot}.
...
The trajectory of the harvester can be represented by a series of continuous tracks, which correspond to different load numbers, working widths and harvest dates, while points within a track are separated by a maximum distance. The function \texttt{CreateHarvestTracks} (\texttt{rasta}) creates such tracks from a \emph{data frame} with \emph{temporally ordered} input records. Note that the previous sentence refers to a data frame, NOT a spatial data frame.
...
\\
\textbf{Task 1}: Inspect the help and code of of the function \texttt{CreateHarvestTracks} (for the latter, type the name of the function on the R prompt) and explain what it does and how it works.
\\
\\
...
\begin{enumerate}
  ...
  \item fill small holes between blocks by swelling and shrinking;
  \item compute for each block the yield per hectare and add it to the spatial polygons data frame
  \item export the spatial polygons data frame for display in Google Earth using either writeOGR or \texttt{kml} from the package \texttt{plotKML}.
\end{enumerate}

Mail your code along with your response to \textbf{task 1} above to Sytze de Bruin.
