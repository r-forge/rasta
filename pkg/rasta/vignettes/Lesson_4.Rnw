...
The trajectory of the harvester can be represented by a series of continuous tracks, which correspond to different loads (identified by load number and date), working widths and harvest dates, while points within a track are separated by a maximum distance. The function \texttt{CreateHarvestTracks} (\texttt{rasta}) creates such tracks from a \emph{data frame} with \emph{temporally ordered} input records. Note that the previous sentence refers to a data frame, NOT a spatial data frame.
