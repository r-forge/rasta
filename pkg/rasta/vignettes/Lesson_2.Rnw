...
\clearpage
\section{Excercise Lesson 2: Under construction}

Provide a script where 
(1) the raster (temperature) is sampled (in a random sample per pixel), 
(2) and the random points are then visualised on top of the raster (try in ggplot)
(3) derive the standard deviation and range of all the temperature values of the sampled points

<<ex, eval=FALSE, echo=TRUE>>=
