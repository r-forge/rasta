...
\author{Jan Verbesselt, Sytze de Bruyn, Loic Dutrieux, Ben De Vries, ...}
...
\begin{itemize} % [<+->]
  \item R
  \item Python (stand-alone or integrated within ArcGIS, QGIS), 
  \item GRASS
\end{itemize}
\emph{Aldo can you help here}
\end{frame}
...
\begin{itemize}
  ...
  \begin{itemize}
   \item {call R from Python using \textbf{RPy}}
...
\begin{frame}[fragile] 
\frametitle{Set Your Working Directory and Load Your Libraries}
\textbf{Set the Working Directory}
...
\begin{frame} [fragile] 
\frametitle{Read and Plot Spatial Data}
The most flexible way to read in a shapefile is by using the $readOGR$ command. This is the only option that will also read in the $.prj$ file associated with the shapefile. NCEAS has a useful summary of the various ways to read in a shapefile: \url{http://www.nceas.ucsb.edu/scicomp/usecases/ReadWriteESRIShapeFiles}
...
%'data/kenyashape.zip')
%unzip('data/kenyashape.zip')
%rm('data/kenyashape.zip')
%#ds <- readOGR(dsn=datdir, layer='kenya')
%@
\end{frame}

\begin{frame}
\frametitle{Excercise Lesson 1: Write you own function}
\textbf{Write a function to visualise data and plots different variables}
\begin{itemize}
\item Submit a documented script to
\item Using data with the 'rasta' package
\item Upload your code to... (Loic? Github?)
\end{itemize}
...
%% Title of the script is Ex_lesson_1_yourname.R
\end{frame}
...
\item  {Visit \url{http://www.r-project.org/} and check out the Manuals i.e an introdutions to R}
%% R spatial project
