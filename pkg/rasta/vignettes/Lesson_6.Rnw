...
In this tutorial, we will explore the raster package and other related packages used for typical raster analyses. We will first look at analysis of RasterLayer objects, exploring functions having to do with raster visualization and raster algebra. We will explore supervised and unsupervised classification in R using the Random Forest and k-means methods. Lastly, we will also make use of a spatially-dependent function, focal(), to implement a raster 'sieve' for post-processing. Since data from the Landsat archive is becoming increasingly important for environmental research and monitoring, this tutorial will focus on the use of these data.
...
One advantage of this function is the fact that the result can be written immediately to file by including the argument \emph{filename="..."}, thus saving memory (especially important when working with large datasets).
...
if (FALSE) {
  # check this Ben
  trainingPoly@data$Test <- sapply(trainingPoly@data$Class, 
                                 FUN = reclass)
...
plot(classes, col=c("dark green", "orange", "light blue"))
if(FALSE) {
  cols <- c("dark green", "orange", "light blue")
  plot(classes, legend = FALSE, col = cols)
  legend("topright", legend = c("forest", "cropland", "wetland"), fill = cols)
}
@
\begin{center}
\begin{figure}[t]
...
%% TODO(Ben): make a diagram explaining the concept below
...
The \emph{focal()} function can be used for a variety of purposes, including other data filters such as median, Laplacian, Sobel, etc. More complex weight matrices can also be computed, such as a Gaussian using the \emph{focalWeight()} function. These methods will not be covered in this tutorial. Check out the documentation at \emph{?focal()} for more information.
...
Perform another Random Forest classification with 6 classes (found in the lulcGewata raster dataset) using all (or some) of the bands used in this tutorial (b2, b3, b4, ndvi and vcf). This time, derive your own training data from the lulcGewata image provided with this package as follows:

%% suggestion: include only comments (with some code) to give some hints to students? Or circulate that as a separate document in class? Or show these hints as separate slides without including in the online vignette?
...
Report on the results by (a) producing a thematic map; (b) giving the output OOB confusion matrix with accuracy statistics; (c) giving your own confusion matrix derived from an overlay of the original LULC raster with your own LULC raster. In this exercise, also address the following questions:

\begin{enumerate}
\item Which classes have the highest accuracy? Lowest? (hint: explore the RandomForest output model object).
