...
An example, with a function that subtract 2 RasterLayers, with the option to plot the resulting RasterLayer, or not.
<<>>=
library(raster)
...
\textit{\textbf{Use of try for error handling}}\\
...
@
\\
\textit{\textbf{Function debugging}}\\
...
\textit{\textbf{Prerequisites}}\\
In order to be able to build package, you must have \textbf{Rtools} installed and properly setup on your system. Refer to the system setup document for more information on how to install Rtools. Once this is done R Studio offers great integration with Rtools, which makes package building really easy. The build panel in R Studio is by default located in the upper right window.

\textit{\textbf{Check and build}}\\
...
And version control offers many benefits that are not tied to packages. The same type of R projects, symbolized by a ''projectName.Rproj'' file can be used to maintain simple scripts and stand alone functions.
