...
During the previous lectures, you saw some general aspects of the R language, such as an introduction to the syntax, object classes, reading of external data and function writting. Today the emphasis of the lecture is mostly oriented toward carrying an R project. Therefore, some recurrent aspects of project development, such as, how to find help, or techniques to organize your code are covered in the lecture.
...
\begin{center}
<<label=cars, fig = TRUE>>=
plot(cars) # The plot function on this type of dataset (class = data.frame, 2 column) automatically generates a scatterplot
@
\end{center}
\caption{Visual representation of the cars dataset}
\label{cars}
\end{figure}

Another famous dataset is the meuse dataset, providing all sorts of spatial variables spread across a part of the meuse watershed.
...
data(meuse)
# The meuse dataset is not by default a spatial object, but its x, y coordinates are part of the data.frame
class(meuse)
...
The sp package help page contain multiple examples of how to explore its meuse built-in dataset. Another example of multiple plots using meuse.grid is given in figure~\ref{meuse2}.

\begin{figure}[H]
\begin{center}
<<label=meuse2, fig = TRUE>>=
...
@
\end{center}
\caption{Another example of spatial data manipulation and spatial plot using the meuse dataset from sp package}
\label{meuse2}
\end{figure}
...
\item Follow the R style guide\footnote{The aim of having a formated style guide is to increase readibility of your code by others. In that regard, identation, use of <- instead of = as assignment operator and placement of curly braces are of particular relevance when writing functions. Proper configuration of your script editor allows you to automatically implement some of these guidelines without having to think too much about it.} \url{http://google-styleguide.googlecode.com/svn/trunk/Rguide.xml}
...
The objectif of this section is to provide a few advices to help you write efficiently function. That is functions that are simple, generic and flexible, so that they integrate well in a processing/analysis chain and can easily be re-used in a slightly different chain if needed.
...
Obviously, the user is expected to pass an object of character vector to x. Otherwise the function will return an error. You can make it not crash by controling the class of the input variable.
For example.
...
Also note that \code{is.character(32) == TRUE} is equivalent to \code{is.character(32)}. Therefore when checking logical arguments, you don't have to use the \code{== TRUE}. As an example, a function may have an argument (\code{plot}) that if set to \code{TRUE} will generate a plot, while if set to \code{FALSE} does not plot. This means that the function certainly contains an if statement. \code{if(plot)} in that case is equivalent to \code{if(plot == TRUE)}, it's just shorter.\\
...
r <- raster(system.file("external/rlogo.grd", package="raster")) # that one is the R logo raster converted to the raster package file format.
r2 <- r # The second RasterLayer is calqued from the initial RasterLayer in order to avoid issues of non matching extent or resolution, etc
r2[] <- (1:ncell(r2)) / 10 # Filling the rasterLayer with new values. The /10 simply makes the result more spectacular
r3 <- minusRaster(r, r2) # simply performs the calculation
@

\begin{center}
<<fig = TRUE>>=
r4 <- minusRaster(r, r2, plot=TRUE) # Performs the claculation and plots the resulting RasterLayer
@
\end{center}

\subsubsection*{Use of \code{try} and debugging your functions}
\emph{\textbf{Use of try for error handling}}\\
...
See the example below that illustrate the use of \code{try} for sequencially calculating frequency on a list of auto-generated RasterLAyers.
...
# So by building a function that includes a try(), we are able to catch the error, allowing the process to continue despite missing/corrupted data.
...
# Note that using a function of the apply family would be a more elegant/shorter way to obtain the same result
(out <- sapply(X=list, FUN=fun, value=-2))

@
\\
\emph{\textbf{Function debugging}}\\

Debugging a single line of code is usually relatively easy; simply double checking the classes of all input arguments often gives good pointers to why the lines crashes. But when writing more complicated functions where objects created within the function are reused later on in that same function or in a nested function, it is easy to lose track of what is happening, and debugging can then become a nightmare. A few trics can be used to make that process less painful. The first thing to investigate right after an error occurs is to run the \code{traceback()} function; just like that without arguments. Carefully reading the return of that function will tell you where exactly in your function the error occured.
...
This sections describes the structure of a simple package, for more information about R packages in general and details of package creation, please refer to the manual \textit{Writing R extensions} \url{http://cran.r-project.org/doc/manuals/R-exts.html}.\\
The default basic structure of a package is as follows:\
2 subdirectories (R and man), NAMESPACE and DESCRIPTION files (Figure~\ref{package}). The \textbf{R directory} includes the package functions of the package, usually each in a separate functionName.R file; Each one of these function should have an associated functionName.Rd file, stored in the \textbf{man directory}. These .Rd files are the functions documentation. \textbf{NAMESPACE} and \textbf{DESCRIPTION} files contain general information about the package (package metadata, dependencies, version number, etc). That is quite a minimalistic package structure; more elaborate packages may include extra subdirectories, such as data, demo or vignette.\\

\subsubsection*{Useful tips for package creation}

The \code{package.skeleton} function will help you get the package structure from a list of sourced functions. (A function opened in the R Studio editor pannel can be sourced by pressing the ''Source'' in the top right hand corner of the pannel)
\code{prompt} creates a tailored documentation (.Rd) file from an existing function.
The project functionalities of the R Studio IDE can greatly assist you in creating a package from scratch, including. Explore the menus and option under Project - New project. 
More package creation tips can be found online \url{http://mages.github.io/R_package_development/#1}

\subsubsection*{Package building}\\
\emph{\textbf{Prerequisites}}\\
In order to be able to build package, you must have \textbf{Rtools} installed and properly setup on your system. Refer to the system setup document for more information on how to install Rtools. Once this is done R Studio offers great integration with Rtools, which makes package building really easy. The build pannel in R Studio is by default located in the upper right window.

\emph{\textbf{Check and build}}\\
...
\textbf{Push} (git only): Add previous changes commited to the local repository, to the remote repository.\\
...
And version control offers many benefits that are not tied to packages. The same type of R projects, symbolyzed by a ''projectName.Rproj'' file can be used to maintain simple scripts and stand alone functions.
...
\begin{enumerate}
  ...
  \item Choose for the option to add a README
...
Note that such packages, that are maintained on git repositories can be installed and compiled directly on the fly using the devtools package. See function \code{install\_github} and \code{install\_bitbucket} for further details.
