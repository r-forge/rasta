...
Requires:
\begin{itemize}
\item Cropping to extent (The second scene will be a bit bigger)
\item Apply the QA2Cloud function again
\item perform replacement
\end{itemize}
...
taravaoComposit <- overlay(cloud, cloud2, taravao, taravao2, fun=compose(cloud, cloud2, taravao, taravao2))
@
