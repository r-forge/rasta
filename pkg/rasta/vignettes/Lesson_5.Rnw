...
\section{Raster objects manipulations}
  ...
  To illustrate the reading and writing of raster files, we will use the data stored in the /inst/extdata/ directory within the rasta package. For the conveignance of that course, we have added data directly to the package; they are read using the \code{}
